...
const Dashboard = () => {
  ...
  function columnFormatter(column, colIndex) {
    return (
      <div style={{ width: 500, fontSize: 12 }}>
  ...
  const fetchColumns = async () => {
    try {
      const res = await axios.get(
        `${apiUrl}/clickhouse/logs_metrics_monitor/columns`
      );
  ...
  const fetchAllData = async () => {
    try {
      const res = await axios.get(
        `${apiUrl}/clickhouse/logs_metrics_monitor/fetch_all_fields`
      );
      setData(res.data);
      return res.data;
    } catch (error) {
  ...
  const applyFilter = async () => {
    try {
      const filteredConditions = filter.filter(
        (condition) =>
          condition.field !== "" &&
          condition.operation !== "" &&
      ...
      const conditionsQueryParam = `conditions=${encodeURIComponent(
        JSON.stringify(filteredConditions)
      )}`;
      const connectorsQueryParam = `connectors=${encodeURIComponent(
        JSON.stringify(connector)
      )}`;
      const aggConditionsQueryParam = `agg_conditions=${encodeURIComponent(
        JSON.stringify([
          { field: "latency", aggregateFunction: "avg" },
          { field: "cost", aggregateFunction: "avg" },
          { field: "latency", aggregateFunction: "quantile", aggVal: "0.95" },
          {
            field: "status_code",
          ...
          { field: "input_token", aggregateFunction: "sum" },
          { field: "output_token", aggregateFunction: "sum" },
        ])
      )}`;
      ...
      const res = await axios.get(
        `${apiUrl}/clickhouse/logs_metrics_monitor/aggregates?${aggConditionsQueryParam}&${conditionsQueryParam}&${connectorsQueryParam}`
      );
  ...
  return loader ? (
    <div style={{ position: "absolute", bottom: "50vh", left: "50vw" }}>
      <Dots />
    </div>
  ) : (
        ...
        <div style={{ width: "90%" }}>
          <div
            style={{
              ...
              borderRadius: 30,
              backgroundColor: "rgba(227, 216, 229, 0.8)",
            }}
          >
            <div
              style={{
          ...
          {loadingMetricUsage ? (
            <>
              <div style={{ fontSize: 15, paddingTop: 15, paddingBottom: 5 }}>
                Data Metrics
              </div>
              ...
              <div style={{ fontSize: 15, padding: 10 }}>
                Requests Per Day Based On Applied Filters
              </div>
          ...
          <div style={{ fontSize: 22, paddingTop: 20, paddingBottom: 10 }}>
            Logs Data
          </div>
