...
const Analytics = () => {
  ...
  const fetchData = async (field, timePeriod) => {
    try {
      ...
      } else field_to_pass = field;
      const response = await axios.get(
        `${apiUrl}/clickhouse/logs_metrics_monitor/fetch_range_data?field_name=${field_to_pass}&date_range=${timePeriod}`
      );
      ...
      switch (field) {
        case "event_id":
          ...
          data.forEach((item) => {
            ...
            reqCountDataMap[timestamp] = reqCountDataMap[timestamp]
              ? reqCountDataMap[timestamp] + 1
              : 1;
          });
          setRequestsData(
            Object.keys(reqCountDataMap).map((timestamp) => ({
        ...
        case "failures":
          ...
          data.forEach((item) => {
            ...
            if (item.status_code != "200") {
              failuresDataMap[timestamp] = failuresDataMap[timestamp]
                ? failuresDataMap[timestamp] + 1
                : 1;
            }
          });

          setFailuresData(
            Object.keys(failuresDataMap).map((timestamp) => ({
              x: parseInt(timestamp),
        ...
        case "success":
          ...
          data.forEach((item) => {
            ...
            if (item.status_code == "200" || item.status_code == "201") {
              successDataMap[timestamp] = successDataMap[timestamp]
                ? successDataMap[timestamp] + 1
                : 1;
            }
          });

          setSuccessData(
            Object.keys(successDataMap).map((timestamp) => ({
              x: parseInt(timestamp),
